...
import com.ichess.game.piece.King;
import com.ichess.game.piece.Pawn;
...
/**
 * Represents a Chess game. The game is by default initialized to the starting
 * position, however you can load a custom position using FEN or PGN. By default
 * a game does not have time control or limits. You can set these time control
 * limits : limit for the entire game, in minutes, and increment in seconds per
 * move. you can check if time runs out using isOutOfTime(). <br>
 * ICHESS web site </a>. <br>
 *
...
public class Game {
    ...
    public int white_material;
    public int black_material;
    public double score;
    ...
    private Game _otherGame = null;

    public void setOtherGame(Game otherGame)
    {
        if (isStarted())
        {
            Log.warning("can't set other game - already started");
            return;
        }
        if (! isBugHouse())
        {
            Log.warning("can't set other game - not bug house");
            return;
        }
	...
	private int _timeForGame = 0, _incrForMove = 0, _timeForMove = 0, _simulColor = Common.COLOR_WHITE, _simulFactor = 1;

	/*
	 * timed game support
	 */
	private int _timeLeftMs[] = new int[Common.COLOR_NUM];
	...
	public void setGameRules(int grules)
	{
		if (isStarted())
		{
			Log.warning("can't change game rules of a started game");
		}
		_grules = grules;
	}
	...
	public void setGameKind(int gkind)
	{
		if (isStarted())
		{
			Log.warning("can't change game kind of a started game");
		}
		_gkind = gkind;
	}
    ...
    public boolean isSevenBoom() {
        return _gkind == Common.GAME_KIND_SEVEN_BOOM;
    }
	...
	/**
	 * Creates a new chess game. the white and black player names are given
	 *
	 * @param whiteName
	...
	public Game(String whiteName, String blackName, int gkind) {
		...
		setAttribute(PGN.STR_DATE, PGN.formatDate(Utils.now()));
		setAttribute(PGN.STR_SITE, Common.ICHESS_SITENAME);
		setAttribute(PGN.STR_RESULT, PGN.STR_RESULT_ONGOING);

		_timeLeftMs[Common.COLOR_WHITE] = 0;
		_timeLeftMs[Common.COLOR_BLACK] = 0;
    ...
    /*
    * creates new game with just last move
     */
    public Game (Game game) {
        copyPositionFrom(game);
        _currentColor = game._currentColor;
        _currentMove = game._currentMove;
        _gkind = game._gkind;
        _ended = game._ended;
        _winner = game._winner;
        _movelist = game._movelist;
        _moveNumber = game._moveNumber > 1 ? 1 : game._moveNumber;
        _currentMove = game._currentMove;
    }
	 ...
	 *
	 * @param color
	 *            - the color that aborted this game.
	 */
	...
	private int analyse(boolean fast) {
		...
		Level debugLevelOnFail = (_currentMove == 0 ? Utils.INFO : Utils.WARNING);
        ...
        if (!fast) {
            if (! isSuicideOrFreePlay())
            {
                ...
                {
                    Log.debug(debugLevelOnFail, "no moving king - analyze failed");
                    ;
                ...
                {
                    Log.debug(debugLevelOnFail, "more then 1 moving king - analyze failed");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
                ...
                {
                    Log.debug(debugLevelOnFail, "no other king - analyze failed");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
                ...
                {
                    Log.debug(debugLevelOnFail, "more then 1 other king - analyze failed");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
            }
            ...
            for (Piece pawn : currentColorPawns)
            {
                ...
                {
                    Log.debug(debugLevelOnFail, "moving pawn on row 1 or 8 - analyze failed");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
            }
            ...
            for (Piece pawn : otherColorPawns)
            {
                if ((pawn.getX() == 1) || (pawn.getX() == 8))
                {
                    Log.debug(debugLevelOnFail, "other pawn on row 1 or 8 - analyze failed");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
            }
		...
		if (!fast && ! isSuicideOrFreePlay())
		{
			if (_currentMove == 0) {
				...
				for (Piece movingPiece : movingPieces) {
					...
					Log.debug(_grules + " piece " + movingPiece.getType() + ":" + movingPiece.getColor() + "," + movingPiece.getPinningPiece()
					    + " " + movingPiece.getX() + "," + movingPiece.getY());
					if (movingPiece.canMoveTo(otherKing.getX(), otherKing.getY(), this)) {
						Log.debug(debugLevelOnFail, "other side in check - analyse failed");
						;
						return Common.RC_GENERAL_FAILURE;
					}
				}
		...
		if (newMoveInfo.isCheck()) {
				...
				for (Piece movingPiece : movingPieces) {
					...
					if (movingPiece.isReachable(checkingX, checkingY)
						...
						if (!oldPiece.canMoveTo(kingX, kingY, this)) { // doesn't check
                            //Log.info("adding move " + movingPiece.isQueen() + checkingX + " " + checkingY);
							newMoveInfo.addValidMove(movingPiece, checkingX, checkingY);
							newMoveInfo.setCheckMate(false);
						}
			...
			if ((!knightCheck) && (!floatCheck)) {
				...
				while ((x != kingX) || (y != kingY)) {
                    ...
                    if (isCrazyOrBugHouse())
                    {
                        // in bughouse always add drop any. we don't know what we'll get in the future
                        ...
                        if (addDropAnyMove)
                        {
                            ...
                            Log.debug("adding drop move by " + droppedPiece.getType() + " color " + Common.GetColor(droppedPiece.getColor()) + " to " + x + "," + y);
						...
						for (int a=1; a<9; a++){ // if he can move anywhere - it won't be check
							for (int b=1; b<9; b++){
								...
								if (saver.isReachable(a, b)) {
                                    // check that not causing new check for grass
                                    if (Math.abs(kingX-a) == 1 && Math.abs(kingY-b) == 1) { // moving near the king
                                        int signX = a-kingX, signY = b-kingY;
                                        for (int r = 1; r<7; r++) {
                                            if (getPieceAt(a+r*signX, b+r*signY) != null) {
                                                if (getPieceAt(a+r*signX, b+r*signY).isGrasshoper()) { // move into check by grass
                                                    move2check = true;
                                                    Log.debug("prevented moving into check!");
                                                    break;
                                                }
                                                else {
                                                    Log.debug("other piece");
                                                    break; /// other piece
                                                }
                                            }
                                        }
                                    ...
                                    if (!move2check) {
                                        Log.debug("adding saver " + checkingX + "," + checkingY + " to " + a + " " + b);
			...
			for (Piece movingPiece : movingPieces) {
				if (movingPiece.isKing()) {
						...
						if (! movingPiece.isMoved()) { // king didn't move
							...
							do {
								...
								*/
                                ...
                                // to check condition 1 we have to move the king to the target square to allow
                                // other pieces (and pawns...) to check if they can "capture" it
                                setPieceAt(kingX, kingY, null);
                                for (int col = leftSquare; col <= rightSquare; col++) {
                                    Piece pieceBetween = getPieceAt(kingX, col);
                                    if (pieceBetween != null) {
                                        if (isFischer() && pieceBetween.isRook())  {
                                            for (Piece otherPiece : otherPieces) {
                                        ...
                                        }  // I am not sure why anyway you move the king to check if opp can move there
                                        continue;
                                    }

									if ((col >= KingDest && col <= KingInit) ) { // condition 1
                                        setPieceAt(kingX, col, movingPiece);
										for (Piece otherPiece : otherPieces) {
											if (otherPiece.canMoveTo(kingX, col, this)) {
												canCastle = false;
												break;
                                        ...
                                        setPieceAt(kingX, col, null);
									}
								}
                                setPieceAt(kingX, kingY, movingPiece);
								...
								if (canCastle) {
                                    ...
                                    Log.debug("gkind " + Common.GAME_KIND_TEXT[getGameKind()] + " allowing O-O-O of " + Common.GetColor(this._currentColor) +
							...
							do {
								...
								if (canCastle) {
                                    ...
                                    Log.debug("grules " + Common.GAME_RULES_TEXT[getGameRules()] + " allowing O-O of " + Common.GetColor(this._currentColor) +
                                            " king from " + kingX + "," + kingY + " to " + KingDest + " or " + RookInit);
								}
							} while (false);
						}
					...
					Log.debug("PIECE:" + movingPiece.getX() + ", " + movingPiece.getY() + " @ " + movingPiece.getColor() + ":" + movingPiece.getType());
					for (int x = 8; x != 0; x--) {
						for (int y = 8; y != 0; y--) {
            ...
            if (isCrazyOrBugHouse())
            {
                ...
                if (addDropAnyMove)
                {
                    ...
                    newMoveInfo.setStaleMate(false);
                    Piece droppedPiece = Piece.create(Common.PIECE_TYPE_DROP_ANY, _currentColor);
                    Log.debug("move " + _currentMove + " color " + _currentColor + " adding drop moves in all squares");
		...
		if (isSuicide())
		{
			...
			if (hasCaptureMove)
			{
				...
				for (Move nextMove : newMoveInfo.getValidNextMoves())
				{
					Log.debug("checking move " + nextMove.getFromX() + "," + nextMove.getFromY() + " - " +
						nextMove.getToX() + "," + nextMove.getToY());

					Piece capturedPiece = nextMove.getCapturedPiece();
					if (capturedPiece != null)
					{
						Log.debug("move " + nextMove.getFromX() + "," + nextMove.getFromY() + " - " +
		...
		if ((Boolean) getAttribute(CHECK_NO_MATERIAL_DRAW)) {
			...
			if ((!newMoveInfo.getHasEnoughMaterial()[Common.COLOR_WHITE])
				...
				_endString = "אין מספיק חומר לשני הצדדים.";
				endGame(Common.COLOR_ILLEGAL);
			}
		}
		...
		if ((Boolean) getAttribute(CHECK_50_MOVES_DRAW)) {
			...
			if (newMoveInfo.getDraw50MovesCount() >= 100) {
				_endString = "50 מסעים ללא הכאת כלי או מסע רגלי.";
				endGame(Common.COLOR_ILLEGAL);
			}
		}
        ...
        newMoveInfo.setFENString(FEN.getFENString(this));
        newMoveInfo.setFENPosition(FEN.getFENPosition(this));
        ...
        if ((Boolean) getAttribute(CHECK_REPEATITION_DRAW)) {
            ...
            for (int moveIndex = 0; moveIndex < _currentMove; moveIndex++) {
                ...
                String newFen = newMoveInfo.getFenStringForDrawTesting(), lastFen = lastMoveInfo.getFenStringForDrawTesting();
                if (newFen.equals(lastFen)) {
                    matches++;
                }
            }
            if (matches == 3) {
                _endString = "חזרה על מסעים.";
                endGame(Common.COLOR_ILLEGAL);
            }
        }
	...
	public void disconnect(int color) {
		_endString = Common.GetColorHeb(color) + " " + "התנתק";
		endGame(Common.OtherColor(color));
	}

	public void drawMutual() {
		_endString = "הוסכם תיקו.";
	...
	private void endGame(int winner) {
		...
		Log.debug("Game " + getName() + " : ended. " + _endString);
	}

	public Piece findPiece(int type, int color) {
		for (int x = 8; x != 0; x--) {
			for (int y = 8; y != 0; y--) {
				Piece piece = getPieceAt(x, y);
				if (piece != null) {
	 ...
	 * if color = 0, get all pieces of that type
	 */
    ...
    public String getCurrentFENString()
    {
        ...
        return getCurrentMoveInfo().getFENString();
    }
	...
	/**
	 * Returns the time increment per move in seconds. 0 means no increment.
	 *
	 * @return The time increment per move in seconds.
	 */
	public int getIncrPerMove() {
		return _incrForMove;
	}
	...
	/**
	 * Returns the game move list, in algebric hebrew notation.
	 *
	 * @return The game move list, in algebric hebrew notation.
	 */
	public String getMoveListAlgHeb() {
		return _movelist.getListAlgHeb();
	}

	public String getMoveListAlgHebFromMove(int fromMove) {
		return _movelist.getListAlgHeb(fromMove);
	}
	...
	/**
	 * Returns the game move list, in hebrew figure notation.
	 *
	 * @return The game move list, in hebrew figure notation.
	 */
	public String getMoveListFigHeb() {
		return _movelist.getListFigHeb();
	}

	public String getMoveListFigHebFromMove(int fromMove) {
		return _movelist.getListFigHeb(fromMove);
	}
	...
	/**
	 * get partial move list - from the given fromMove to the end of the movelist
	 * @param fromMove
	 * @return
	 */
	...
	/**
	 * get partial move list - from the beginning to the given toMove
	 * @param toMove
	 * @return
	 */
    ...
    private int timeSinceClockStarted(int color)
    {
        return Math.max(0, (int)(Utils.nowInMs() - _clockStarted[color]));
    }

    /**
     * return time left for the color to play in games where there is a time for move enforcement,
     * based on the time for move enforcement only !
     * @param color
     * @return 0 if its not the color turn, clock was not started, or there is no time for move enforcement.
     */
    public int timeLeftForMove(int color)
    {
        ...
        return ( _timeForMove * (int) Utils.MS_IN_SECOND ) - timeSinceClockStarted(color);
    }
	...
	/**
	 * Returns the time left, in millisecond, for the given color. (not including elapsed time since clock started)
	 *
	 * @param color
	 * @return The time left, in millisecond, for the given color.
	 */
	public int getTimeLeftMs(int color) {
		return _timeLeftMs[color];
	}

	/**
	 * Returns the time limit for the entire game in minutes. 0 means there is
	 * no time limit.
	 *
	...
	/**
	 * Returns the time limit for each move, in seconds. 0 if there is no limit.
	 *
	 * @return The time limit for each move, in seconds
	 */
	public int getTimeLimitForMove() {
		return _timeForMove;
	}
	...
	public void initialPosition() {
		...
		switch (_gkind)
		{
            ...
            case Common.GAME_KIND_FREEPLAY:
            case Common.GAME_KIND_SUICIDE:
                initPosition = FEN.FEN_INITIAL_POS_NO_CASTLE;
            break;
		...
		FEN.loadGame(this, initPosition , true, _gkind);
	...
	public boolean isOutOfTime(int color) {
        ...
        Log.debug("Game : " + getName() + " : time lefts " + _timeLeftMs[Common.COLOR_WHITE] + " , " +
            _timeLeftMs[Common.COLOR_BLACK]);
		...
		if (color == getCurrentColor()) {
            ...
            int timeSinceClockStarted = timeSinceClockStarted(color);
            if ( _timeForGame > 0 ) {
                // check time limit for game
                // check the current player - if the remaining time -
                // check the time left, and also check the time left with respect to
                // thinking in the current move
                if (_timeLeftMs[color] < 0) {
                    Log.info("Game : " + getName() + " : " + Common.GetColor(color) + " is out of time.");
                    return true;
                }

                if (timeSinceClockStarted > _timeLeftMs[color]) {
                    Log.info("Game : " + getName() + " : " + Common.GetColor(color) + " is out of time.");
                    return true;
                }
            }
            ...
            if (_timeForMove != 0)
            {
                if ( timeSinceClockStarted > (_timeForMove * Utils.MS_IN_SECOND ))
                {
                    Log.info("Game : " + getName() + " : " + Common.GetColor(color) + " is out of time.");
                    return true;
                }
			...
			return (_timeLeftMs[Common.OtherColor(color)] < 0);
		}
	}
	...
	/**
	 * Returns true if the game has time limit. this means return true if there
	 * is a time limit for game or increment per move (or both).
	 *
	 * @return true if the game has time limit. otherwise false.
	 */
	public boolean isTimed() {
		return ((_timeForGame > 0) || (_incrForMove > 0) || (_timeForMove > 0));
	}

	public boolean isWhiteOrBlack(String name) {
		Utils.AssertNull(name);
		return (name.equals(_whiteName)) || (name.equals(_blackName));
	}

	private int movePiece(int fromX, int fromY, int toX, int toY) {
		Piece fromPiece = _board[fromX - 1][fromY - 1];
		Utils.AssertNull(fromPiece);
		setPieceAt(fromX, fromY, null);
		setPieceAt(toX, toY, fromPiece);
		return Common.RC_OK;
	}

	public boolean offerDraw(int color) {
	...
	public void pauseGame() {
		stopClock(_currentColor);
		_isPaused = true;
	}

    public int playMove(Move move) {
        return playMove(move, false);
    }

	public int playMove(Move move, boolean fast) {

		Utils.AssertNull(move);

        if (Common.RC_OK != analyse(fast)) {
            Log.warning("analyse failed.");
            ;
            return Common.RC_GENERAL_FAILURE;
        }

        if (_ended) {
            Log.info("can't play move. game ended.");
            ;
            return Common.RC_GENERAL_FAILURE;
        }
        ...
        if ((testMove == null) || ( !fast && (testMove != move)) ) {
            Log.warning("move not in valid moves list");
            ;
            return Common.RC_GENERAL_FAILURE;
        }

        if (isPaused()) {
            Log.warning("can't play move now. game is paused");
            ;
            return Common.RC_GENERAL_FAILURE;
        }
        ...
        if (move.isDropMove())
        {
            ...
            if (droppedPiece == null) {
                Log.warning("can't find dropped piece " + move.getAdditionalPieceTypeInfo());
                return Common.RC_GENERAL_FAILURE;
            }
            Log.debug("move " + _currentMove + " color " + Common.GetColor(_currentColor) + " drop move " +
                    droppedPiece.getType() + " at " + toX + "," + toY);
            if (! droppable.remove(droppedPiece))
            {
                Log.warning("could not remove droppable piece");
            }
        ...
        else
        {
            ...
            if (movingPiece == null) {
                Log.warning("moving piece is null");
                ;
                return Common.RC_GENERAL_FAILURE;
            }

            if (movingPiece.getColor() != _currentColor) {
                Log.warning("moving piece have incorrect color");
                return Common.RC_GENERAL_FAILURE;
            }
            ...
            if (capturedPiece != null) {
                ...
                if (capturedPiece.isKing() && ! isSuicideOrFreePlay()) {
                    ...
                    Log.warning("can't capture a king");
                    ;
                    return Common.RC_GENERAL_FAILURE;
                }
                ...
                if (capturedPiece.getColor() == _currentColor) {
                    if (isFischer() && capturedPiece.isRook() && movingPiece.isKing()) {
                        ...
                        Log.warning("can't capture same color");
                        ;
                        return Common.RC_GENERAL_FAILURE;
                    }
                }
            ...
            if ((movingPiece.isPawn()) && (toX == promotionSquare[_currentColor])) {
                Log.debug("promotion move. promotion piece is " + move.getAdditionalPieceTypeInfo());
            ...
            if (isSevenBoom() && movingPiece.isPawn() &&
                 ( (_currentColor == Common.COLOR_WHITE && toX == 7) ||  (_currentColor == Common.COLOR_BLACK && toX == 2) ) ) {
                Log.debug("Seven promotion move. promotion piece is " + move.getAdditionalPieceTypeInfo());
                Piece promotedPiece = Piece.create(move.getAdditionalPieceTypeInfo(), _currentColor);
                promotedPiece.setPromoted();
                setPieceAt(fromX, fromY, null);
                setPieceAt(toX, toY, promotedPiece);
            ...
            if ((movingPiece.isPawn() && (Math.abs(fromX - toX) == 2))) {
                Piece isOtherPawn = getPieceAt(toX, toY + 1);
                if ((isOtherPawn != null) && isOtherPawn.isPawn() && isOtherPawn.isColor(Common.OtherColor(_currentColor))) {
                    setEpPawn((Pawn) movingPiece);
                }
                isOtherPawn = getPieceAt(toX, toY - 1);
                if ((isOtherPawn != null) && isOtherPawn.isPawn() && isOtherPawn.isColor(Common.OtherColor(_currentColor))) {
                    setEpPawn((Pawn) movingPiece);
                }
            } else {
        ...
        _reqPause[Common.COLOR_WHITE] = false;
        _reqPause[Common.COLOR_BLACK] = false;
		move.setTimePlayed(Utils.nowInMs());

		// update time (unless its first or second move) of current color (and only then switch colors)
		if (isTimed()) {
			stopClock(_currentColor);
			int clockDuration = getClockDuration(_currentColor);
			if (_currentMove < 2 && clockDuration < 10*Utils.MS_IN_SECOND) { // return the time for the first move (for B&W) if it is less than 10 sec
				clockDuration = 0;
			}
            move.setMoveTime(clockDuration);
			_timeLeftMs[_currentColor] -= clockDuration;
			resetClock(_currentColor);

            int factor = 1;
            if ( _currentColor == _simulColor) {
                factor = _simulFactor;
            }
            _timeLeftMs[_currentColor] += (_incrForMove * Utils.MS_IN_SECOND * factor);

			Log.debug(" Game " + getName() + " : Time Left W " + (_timeLeftMs[Common.COLOR_WHITE] / Utils.MS_IN_SECOND) + " B "
				+ (_timeLeftMs[Common.COLOR_BLACK] / Utils.MS_IN_SECOND));

			startClock(Common.OtherColor(_currentColor));
		}
		...
		Log.debug(" Game " + getName() + " : " + ((_currentMove + 1) / 2) + "." + (((_currentMove % 2) == 0) ? ".. " : " ")
                + move.getNameAlg() + " . Time left : [ " + _timeLeftMs[Common.COLOR_WHITE] + " ] , [ "
                + _timeLeftMs[Common.COLOR_BLACK] + " ]");
     ...
     *
     * @param moveStr
     *            move string in either algebric or numeric english notation.
     * @return true if the move was successfully played
     */
	public int playMove(String moveStr) {

		Utils.AssertNull(moveStr);

		analyse();

		Move move = Notation.getMove(this, moveStr);
		if (move == null) {
			Log.warning("could not parse move '" + moveStr + "' from player " + getMovingPlayerName());
			return Common.RC_GENERAL_FAILURE;
		}
	 ...
	 *
	 * @param color
	 *            the color that resigned this game.
	 * @return
	 */
	public int resign(int color) {
		_endString = Common.GetColorHeb(color) + " " + "נכנע";
		endGame(Common.OtherColor(color));
		return Common.RC_OK;
	}
    ...
    public void otherGameEnded() {
        switch (_otherGame.getWinner())
        {
            ...
            default:
                Log.warning("bughouse other game did not end");
                return;
        }
        _endString = "שח שוודי : המשחק השני הסתיים";
    }


    public void resumeGame() {
		startClock(_currentColor);
		_isPaused = false;
        _reqPause[Common.COLOR_WHITE] = false;
        _reqPause[Common.COLOR_BLACK] = false;
	}
	 ...
	 * @return
	 */
	int setCastlingAvailability(int color, int castle, boolean available) {
		if (isStarted()) {
			Log.warning("can't set castling. game allready started");
			return Common.RC_GENERAL_FAILURE;
		}
	...
	/**
	 * Sets time increment, per move, in seconds. This should be set together
	 * with setTimeLimitForGame.
	 *
	 * @param incrInSeconds
	 *            increment for each move, in seconds.
	 * @return true on success, otherwise false.
	 */
	public boolean setIncrPerMove(int incrInSeconds) {
		if (incrInSeconds < 0) {
			Log.warning("negative incrInSeconds");
			return false;
		}

		_incrForMove = incrInSeconds;
		...
		_timeLeftMs[Common.COLOR_WHITE] = (int)(_timeForGame * Utils.MS_IN_MINUTE);
		_timeLeftMs[Common.COLOR_BLACK] = (int)(_timeForGame * Utils.MS_IN_MINUTE);

		return true;
	}

	/*
	 * finds the first piece on the board that matches the given type and color
	 */

	int setMoveNumber(int move) {
		if (isStarted()) {
			Log.warning("can't set move number. game allready started");
			return Common.RC_GENERAL_FAILURE;
		}
    ...
    public King getEnemyKing() {
        return _king[Common.OtherColor(_currentColor)];
    }

	int setPieceAt(int x, int y, Piece piece) {
		_board[x - 1][y - 1] = piece;
		if (piece != null) {
			piece.setX(x);
			piece.setY(y);
	...
	public void setReqDraw(int color) {
		_reqDraw[color] = true;
	}
	...
	public int setStartingColor(int color) {
		if (isStarted()) {
			Log.warning("can't set color. game allready started");
			return Common.RC_GENERAL_FAILURE;
		}
    ...
    public boolean setSimulParameters(int simulColor, int factor)
    {
        if (factor <= 0)
        {
            Log.warning("negative white simul factor");
            return false;
        }
        _simulFactor = factor;
        _simulColor = simulColor;
        if (isStarted()) {
            // can't update actual clock
            return true;
        }
        _timeLeftMs[Common.COLOR_WHITE] = (int)(_timeForGame * Utils.MS_IN_MINUTE);
        _timeLeftMs[Common.COLOR_BLACK] = (int)(_timeForGame * Utils.MS_IN_MINUTE);
        if (_simulColor == Common.COLOR_WHITE) {
            _timeLeftMs[Common.COLOR_WHITE] *= _simulFactor;
        } else {
            _timeLeftMs[Common.COLOR_BLACK] *= _simulFactor;
        }
        return true;
    }
	 ...
	 * @param timeleft
	 */
	public void setTimeLeftMs(int color, long timeleft) {
		_timeLeftMs[color] = (int) timeleft;
	}

	/**
	 * Sets time limit for the entire game in minutes. The game must not be
	 * started. Set to 0 to disable time limit for game.
	 *
	 * @param timeInMinutes
	 *            time limit for the entire game, in minutes
	 * @return true on success, otherwise false.
	 */
	public boolean setTimeLimitForGame(int timeInMinutes) {

		if (timeInMinutes < 0) {
			Log.warning("negative timeInMinutes");
			return false;
		}

		_timeForGame = timeInMinutes;
		updateTimeControlPGNTag();
		if (isStarted()) {
			// can't update actual clock
			return true;
	...
	public boolean setTimeLimitForMove(int timeInSeconds) {
		...
		if (timeInSeconds < 0) {
			Log.warning("negative timeInSeconds");
			return false;
		}

		_timeForMove = timeInSeconds;
	...
	public void startClock(int color) {
		_clockStarted[color] = Utils.nowInMs();
	}

    public long getClockStartedTime() {
        return _clockStarted[_currentColor];
    }
	...
	public void stopClock(int color) {
		if (_clockStarted[color] == 0) {
			return;
		}
		_clockDuration[color] += timeSinceClockStarted(color);
		_clockStarted[color] = 0;
	}
    ...
    public void calcMaterials(){
        calcMaterials(FEN.getFENPosition(this));
    }

    public void calcMaterials(String fen){
        //Log.info("calculating material for fen:" + fen);
        char c;
        white_material = 0;
        black_material = 0;
        ...
        for (int i=0, n=fen.length(); i<n; i++) {
            c = fen.charAt(i);
            switch(c){
                case 'p': black_material++;
                    break;
                case 'P': white_material++;
                    break;
                case 'n': black_material += 3;
                    break;
                case 'N': white_material += 3;
                    break;
                case 'b': black_material += 3;
                    break;
                case 'B': white_material += 3;
                    break;
                case 'r': black_material += 5;
                    break;
                case 'R': white_material += 5;
                    break;
                case 'q': black_material += 9;
                    break;
                case 'Q': white_material += 9;
                    break;
                case 'k': black_material += 300;
                    break;
                case 'K': white_material += 300;
                    break;
                case 'g': black_material += 2;
                    break;
                case 'G': white_material += 2;
                    break;
                case 'a': black_material += 10;
                    break;
                case 'A': white_material += 10;
                    break;
                case 'c': black_material += 8;
                    break;
                case 'C': white_material += 8;
                    break;
                case ' ': // end of fen material
                    return;
            }
        }
    }
	...
	/**
	 * Take back the last move player, if present.
	 */
	public void takeback() {


		if (_currentMove == 0) {
			Log.info("no takeback - game at first move");
			return;
		}
        ...
        else
        {
            ...
            if (capturedPiece != null)
            {
                if (! getCapturedPieces(_currentColor).remove( capturedPiece )) {
                    Log.warning("could not find " + Common.GetColor(_currentColor) + " captured piece " + capturedPiece);
                }
            }
        }
		...
		if (move != null) {
            ...
            // check if pawn made a 2 jump
			if (movedPiece.isPawn()) {
				if (Math.abs(move.getToX() - move.getFromX()) == 2) {
                    // check if there is another pawn nearby
                    Piece isOtherPawn = getPieceAt(move.getToX(), move.getToY() + 1);
                    if ((isOtherPawn != null) && isOtherPawn.isPawn() && isOtherPawn.isColor(_currentColor)) {
                        setEpPawn((Pawn) movedPiece);
                    }
                    isOtherPawn = getPieceAt(move.getToX(), move.getToY() - 1);
                    if ((isOtherPawn != null) && isOtherPawn.isPawn() && isOtherPawn.isColor(_currentColor)) {
                        setEpPawn((Pawn) movedPiece);
                    }
				}
		...
		Log.debug(" Game " + getName() + " : taking back move " + lastMove.getNameAlg());
	...
	public int takebackToMove(int moveNumber) {
	 ...
	 *
	 * @param color
	 *            the color that ran out of time.
	 */
	public void timeOut(int color) {
		...
		if (getCurrentMoveInfo().getHasEnoughMaterial()[otherColor]) {
			winner = otherColor;
			_endString = Common.GetColorHeb(color) + " " + "עבר את הזמן";
			Log.info("Game " + getName() + " ended : " + Common.GetColor(color) + " out of time.");
		} else {
			winner = Common.COLOR_ILLEGAL;
			_endString = Common.GetColorHeb(color) + " " + "עבר את הזמן" + ", אבל ל" +
                    Common.GetColorHeb(otherColor) + " " + "אין מספיק חומר לכפות מט";
			Log.info("Game " + getName() + " ended : " + Common.GetColor(color) + " out of time. but no material for "
	...
	private void updateTimeControlPGNTag() {
		if ((_timeForGame > 0) || (_incrForMove > 0)) {
			if (_incrForMove == 0) {
				setAttribute(PGN.STR_TIME_CONTROL, String.valueOf(_timeForGame * 60));
			} else {
				setAttribute(PGN.STR_TIME_CONTROL, String.valueOf(_timeForGame * 60) + "+" + String.valueOf(_incrForMove));
			}
		} else if (_timeForMove > 0) {
			setAttribute(PGN.STR_TIME_CONTROL, "*" + String.valueOf(_timeForMove));
		}
	}
